...
def update_demand_analysis_graph(quantity_sales_radio, time_window, graph_data_tab, filtered_data):
    ...
    df_sales_filtered = pd.merge(df_lastyear, filtered_data, on=['sku', 'channel', 'warehouse_code', 'region'], how='inner')
    ...
    if graph_data_tab == 'da-tab-1':
        ...
        df_month1['mom_change'] = (df_month1[quantity_sales_radio] - df_month1_prev_year[quantity_sales_radio]) / df_month1_prev_year[quantity_sales_radio]
        ...
        fig_winners = px.bar(winners, x='date', y=quantity_sales_radio, color='sku_warehouse', title='Growth Drivers - MoM Change')
        fig_losers = px.bar(losers, x='date', y=quantity_sales_radio, color='sku_warehouse', title='Losers - MoM Change')
        ...
        # Create the treemap
        df_tree_map=df_tree_map.groupby(['channel','region','level_1','sku'])[quantity_sales_radio].sum().reset_index()
